...
//   // Группировка и сортировка статей по категориям
//   const groupedPosts = blogs.blog
//     .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()) // Сортировка по дате (свежие первыми)
//     .reduce((acc, post) => {
//       if (!acc[post.name]) {
//         acc[post.name] = [];
//       }
...
//   // Функция для рендеринга статей категории
//   const renderCategoryPosts = (category: string, posts: Blog[]) => {
//     const latestPosts = selectedCategory ? posts : posts.slice(0, 5); // Все статьи или 5 последних
//     const bigPost = latestPosts[0]; // Первая статья — большое фото слева
//     const smallPosts = latestPosts.slice(1, 5); // Следующие 4 — маленькие справа
...
//               <div key={index} className={styles.smallPost}>
//                 <Image
//                   src={post.urlphoto}
//                   alt={post.title}
//                   width={150}
...
//         {selectedCategory
//           ? renderCategoryPosts(selectedCategory, groupedPosts[selectedCategory])
